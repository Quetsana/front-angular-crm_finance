      ...
      <div class="card animate-fade-in delay-200">
        <div class="p-4 flex items-center justify-between gap-3 flex-wrap border-b" style="border-color:var(--border-color)">
          <div class="relative">
            ...
            <input class="form-input pl-9 w-64" type="text"
              [(ngModel)]="search" placeholder="Search transactions..."
              (ngModelChange)="onSearch()">
          </div>
          <div class="flex gap-2 items-center">
            <span class="text-sm" style="color:var(--text-muted)">
              {{ paginatedTxns().length }} / {{ filteredTxns().length }} records
            </span>
            <select class="form-input form-select w-auto text-sm" [(ngModel)]="pageSize" (change)="page.set(1)">
        ...
        @if (loading()) {
          <table class="data-table"><tbody>
            <app-skeleton type="table-row" [rows]="[1,2,3,4,5]" [cols]="[1,2,3,4,5,6]"></app-skeleton>
          </tbody></table>
        } @else {
              ...
              <tbody>
                @for (tx of paginatedTxns(); track tx.id) {
                  <tr class="animate-fade-in">
                    ...
                    <td class="text-xs font-mono" style="color:var(--text-muted)">{{ tx.reference ?? '—' }}</td>
                    <td class="text-sm" style="color:var(--text-secondary)">{{ tx.date | date:'MMM d, y' }}</td>
      ...
      <div class="offcanvas-panel animate-fade-right p-5">
        ...
        <div class="flex flex-col gap-4">
          <div>
            ...
            <select class="form-input form-select" [(ngModel)]="filterType" (change)="applyFilters()">
          ...
          <div>
            ...
            <select class="form-input form-select" [(ngModel)]="filterCategory" (change)="applyFilters()">
          ...
          <div>
            ...
            <input class="form-input" type="date" [(ngModel)]="filterDateFrom" (change)="applyFilters()">
          </div>
          <div>
            <label class="form-label">Date To</label>
            <input class="form-input" type="date" [(ngModel)]="filterDateTo" (change)="applyFilters()">
          </div>
          <div>
            <label class="form-label">Min Amount ($)</label>
            <input class="form-input" type="number" [(ngModel)]="filterMinAmt" (change)="applyFilters()" placeholder="0">
          </div>
          <div>
            <label class="form-label">Max Amount ($)</label>
            <input class="form-input" type="number" [(ngModel)]="filterMaxAmt" (change)="applyFilters()" placeholder="9999">
          </div>
        </div>
...
export class TransactionsComponent implements OnInit {
  ...
  readonly filteredTxns = computed(() => {
    ...
    if (this.filterType)     txns = txns.filter(t => t.type === this.filterType);
    if (this.filterCategory) txns = txns.filter(t => t.category === this.filterCategory);
    if (this.filterDateFrom) txns = txns.filter(t => t.date >= this.filterDateFrom);
    if (this.filterDateTo)   txns = txns.filter(t => t.date <= this.filterDateTo);
    if (this.filterMinAmt)   txns = txns.filter(t => t.amount >= +this.filterMinAmt);
    if (this.filterMaxAmt)   txns = txns.filter(t => t.amount <= +this.filterMaxAmt);
    return txns;
  });
  ...
  ngOnInit(): void {
    this.route.queryParams.subscribe(p => {
      if (p['type']) { this.filterType = p['type']; }
    });
  ...
  clearFilters(): void {
    ...
    this.filterDateTo = this.filterMinAmt = this.filterMaxAmt = '';
    this.page.set(1);
  }
